...
Portfolio Construction 檢查（§9A，門檻定義於 `INVESTMENT_POLICY.md` §2）：
  單一個股是否在限額內？__　板塊是否在限額內？__　主題是否在限額內？__
  （門檻隨帳戶風險偏好調整，任何例外須明確記錄）
相關性風險：
組合限制：
```
...
`不適用`（附原因）或針對每個 EXECUTE 候選／每筆釋出資金重複：

```text
決定：Deploy／Rotate／Reserve Cash／Wait
目標股票代號（若 Deploy／Rotate，須為第 12 節已發布之 EXECUTE 候選）：
來源股票代號（若 Rotate——即第 13 節釋出資金之股票代號）：
股數（若 Deploy／Rotate——本引擎計算，上限為 Risk Engine 已核准之
  倉位大小，不可超過）：
金額：
等待類別（若 Wait）：Reserve for Earnings／
  Reserve for Volatility／Wait for Better Setup
原因：
```

`Deploy`／`Rotate` 只可投向第 12 節已發布之 EXECUTE 候選——本引擎不會自行
選擇候選，亦不會超過 Risk Engine 已核准之倉位大小。若釋出資金來源為本次
第 13 節之特定倉位，使用 `Rotate`；否則使用 `Deploy`。若投入後現金將低於
`INVESTMENT_POLICY.md` §4 之最低比例，改為 `Reserve Cash`。

若資金／候選／Risk 核准倉位資料不完整：
`CAPITAL ALLOCATION INCOMPLETE — DATA REQUIRED`
（`CAPITAL_ALLOCATION_ENGINE.md` §7），不影響第 12 節已發布之最終結果。
...
- [ ] 第 13 節投資組合行動計劃已呈現，且每個股票代號的 Decision 均與第 12
      節 Master Decision 已發布的結果一致，未在此另行決定，且無任何
      EXECUTE 股票代號出現在第 13 節
- [ ] 第 14 節資金配置中，每筆 Deploy／Rotate 的目標股票代號均為第 12 節
      已發布之 EXECUTE 候選，未於此另行創造機會，且股數未超過 Risk
      Engine 已核准之倉位大小
